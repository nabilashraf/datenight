...
-(void)markDates:(NSArray *)dates {
    ...
    for (int i = 0; i<[dates count]; i++) {
        [colors addObject:COMMON_COLOR_RED];
    }
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextMoveToPoint(context, xmargin+arrowSize/1.5, ymargin+12);
   ...
   // CGContextSetFillColorWithColor(context,COMMON_COLOR_RED.CGColor);
    ...
    CGContextAddLineToPoint(context,self.frame.size.width-(xmargin+arrowSize/1.5), ymargin+12);
    ...
    NSMutableArray *weekdays = [[NSMutableArray alloc] initWithObjects:@"Su",@"Mo",@"Tu",@"We",@"Th",@"Fr",@"Sa", nil];
    ...
    CGContextSetFillColorWithColor(context, COMMON_COLOR_GRAY.CGColor);
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        if (selectedDate && i==selectedDateBlock) {
            ...
            CGContextSetFillColorWithColor(context,COMMON_COLOR_DATE.CGColor);
    ...
    for (int i = 0; i<[self.markedDates count]; i++) {
        ...
        int targetY = kVRGCalendarViewTopBarHeight + targetRow * (kVRGCalendarViewDayHeight+2) + 25;
...
-(id)init {
    ...
    if (self) {
        ...
        labelCurrentMonth.font = [UIFont fontWithName:@"HelveticaNeue-thin" size:18];
        labelCurrentMonth.textColor = COMMON_COLOR_RED;
